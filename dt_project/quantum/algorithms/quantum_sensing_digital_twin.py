...
class QuantumSensingTheory:
    ...
    def quantum_advantage_factor(self, 
                                 ...
                                 hl_precision: float = None,
                                 sql_precision: float = None) -> float:
        """
        Calculate quantum advantage factor: SQL/HL ratio
        
        From Degen et al. (2017): Quantum advantage scales as √N
        
        Args:
            num_measurements: Number of measurements for calculation
            hl_precision: Heisenberg limit precision (optional, for direct comparison)
            sql_precision: SQL precision (optional, for direct comparison)
        
        Returns:
            Factor of improvement over classical (should be √N)
        """
        # Support both direct precision values and calculation from measurements
        if hl_precision is not None and sql_precision is not None:
            return sql_precision / hl_precision
        elif num_measurements is not None:
            ...
            # SQL = 1/√N, HL = 1/N
            # Advantage = SQL/HL = (1/√N) / (1/N) = N/√N = √N
            return sql / hl
        else:
            # Default to 100 measurements
            return self.quantum_advantage_factor(num_measurements=100)
...
class QuantumSensingDigitalTwin:
    ...
    def __init__(self, 
                 ...
                 n_qubits: int = None):  # Legacy parameter support
        """
        ...
        Args:
            ...
            n_qubits: Legacy parameter name (use num_qubits instead)
        """
        # Support legacy n_qubits parameter
...
# Factory functions
def create_quantum_sensing_twin(num_qubits: int = 4,
                                modality: SensingModality = SensingModality.PHASE_ESTIMATION) -> QuantumSensingDigitalTwin:
    """
    Create a Quantum Sensing Digital Twin
    
    Based on Degen et al. (2017) and Giovannetti et al. (2011)
    
    Args:
        num_qubits: Number of quantum sensors
        modality: Type of sensing
        
    Returns:
        Configured QuantumSensingDigitalTwin
    """
    return QuantumSensingDigitalTwin(num_qubits=num_qubits, modality=modality)


# Aliases for backward compatibility
n_qubits = 'num_qubits'  # Legacy parameter name support
