...
class QuantumDigitalTwinCore:
    ...
    async def update_twin_state(self, twin_id: str, new_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Update a quantum twin's state with new data
        
        Args:
            twin_id: ID of the twin to update
            new_data: Dictionary with new state data
        ...
        # Update the twin's data
        for key, value in new_data.items():
            if hasattr(twin, key):
                setattr(twin, key, value)
        
        # Re-encode to quantum state
        await twin.update_from_sensors(new_data)
        
        return {
            'status': 'success',
            'twin_id': twin_id,
            'updated_fields': list(new_data.keys()),
            'new_fidelity': twin.quantum_state.fidelity if twin.quantum_state else None
        }
    
    async def measure_quantum_advantage(self, twin_id: str) -> Dict[str, Any]:
        """
        Measure the quantum advantage of a specific twin
        
        Args:
            twin_id: ID of the twin to measure
            
        Returns:
            Dictionary with quantum advantage metrics
        """
        if twin_id not in self.twins:
            raise ValueError(f"Quantum twin {twin_id} not found")
        
        twin = self.twins[twin_id]
        return await self._measure_quantum_advantage(twin)
    ...
    def get_managed_twins(self) -> List[str]:
        """Get list of managed twin IDs"""
...
class QuantumSensorNetwork:
    ...
    async def process_sensor_data(self, entity_id: str, sensor_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if entity_id in self.sensors:
            # Extract only numeric values for the mean calculation
            numeric_values = []
            for v in sensor_data.values():
                if isinstance(v, (int, float)):
                    numeric_values.append(v)
                elif isinstance(v, (list, np.ndarray)):
                    # Flatten and take numeric values from lists/arrays
                    flat = np.array(v).flatten()
                    numeric_values.extend(flat.tolist())
            
            mean_value = np.mean(numeric_values) if numeric_values else 0.0
